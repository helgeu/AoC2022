...
let doMove (dict:Dictionary<string, list<string>>) m =
...
let mover = doMove sdict


i |> getMoveInstructions |> Seq.fold (fun s e -> doMove s e) sdict
...
i |> Seq.filter (fun s -> s.Contains("move")) |> getMoveInstructions |> Seq.fold (fun s e -> doMove s e) sdict
